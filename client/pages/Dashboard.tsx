      ...
      try {
        const [
          ...
          api.get("/movimentacoes/medicamentos", {
            params: { days: 7, page: 1, limit: 600 },
          }),
        ...
        const noStockItems = stockList.data.filter(
          (i) => i.st_quantidade === "critical",
        );

        const itemsInStockWarning = stockList.data.filter(
          (i) => i.st_quantidade === "low",
        );

        const expiredItems = stockList.data.filter(
          (i) => i.st_expiracao === "expired" && i.quantidade > 0,
        );

        const expiringSoonItems = stockList.data.filter(
          (i) =>
